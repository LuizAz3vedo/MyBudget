            ...
            dbc.Col([
                dbc.Col([
                    ...
                    html.Div(
                        ...
                        html.Legend('Período de Analise', style={'margin-top': '10px'}),
                        dcc.DatePickerRange(
...
def update_output(data_despesa, data_receita, despesa, receita):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_acum.index, y=df_acum['Acum'], name='FLuxo de Caixa', mode='lines+markers', line=dict(color='royalblue', width=2)))
...
def pie_receita(data_receita, receita):
    ...
    fig.update_traces(title = "Receitas")
...
def pie_despesa(data_receita, receita):
    df = pd.DataFrame(data_receita)
    df = df[df["Categoria"].isin(receita)]

    fig = px.pie(df, values=df.Valor, names=df.Categoria, hole=0.2)
    fig.update_traces(title="Despesas")
    fig.update_layout(margin=graph_margin, height=350)
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
